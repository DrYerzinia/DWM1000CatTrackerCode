...
int dwt_configure(dwt_config_t *config){
    ...
    dw1000local.sysCFGreg |= (SYS_CFG_PHR_MODE_11 & (((uint32)config->phrMode) << 16)) ;
    ...
    dw1000local.txFCTRL = (((uint32)config->txPreambLength) | config->prf) << 16;
    dw1000local.txFCTRL |= (((uint32)config->dataRate) << TX_FCTRL_TXBR_SHFT) | TX_FCTRL_TR; // Always set ranging bit !!!
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
static uint32 _dwt_otpsetmrregs(int mode){
 ...
 * no return value
 */
static void _dwt_disablesequencing(void) // Disable sequencing and go to state "INIT"
{
    _dwt_enableclocks(FORCE_SYS_XTI); // Set system clock to XTI
